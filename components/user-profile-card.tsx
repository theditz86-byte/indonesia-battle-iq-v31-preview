...
export function UserProfileCard({ participant, ownEntry, scope }: { participant: BattleParticipant | null; ownEntry?: BattleEntry; scope: Scope }) {
  ...
  const actionLabel = hasActive ? "Lanjutkan tes Battle" : freeRemaining > 0 ? `Mulai tes · sisa gratis ${freeRemaining}` : paidCredits > 0 ? `Mulai tes · tiket tersedia ${paidCredits}` : "Buka 1 percobaan · Rp5.000"
