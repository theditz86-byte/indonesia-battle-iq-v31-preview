...
async function buildCard(props:Props){
  const profile=await getProfile(); const official=props.rankedAttempt !== false; const ranks=official?await getRanks(props,profile):{}
  ...
  const topPercent=official&&props.nationalRank&&props.leaderboardTotal?Math.max(1,Math.ceil((props.nationalRank/props.leaderboardTotal)*100)):null;const localRank=official?(ranks.regency||ranks.district||ranks.province):undefined;const localLabel=ranks.regency?`Kab. ${props.regencyName||""}`:ranks.district?(props.districtName||"Kecamatan"):ranks.province?(props.provinceName||"Provinsi"):"Wilayah";const nationalLabel=official&&props.nationalRank?`#${props.nationalRank}`:topPercent?`Top ${topPercent}%`:"Top —"
  const cardW=230,gap=18,start=53,y=1248;panel(ctx,start,y,cardW,"◉",props.nickname.length>12?props.nickname.slice(0,11)+"…":props.nickname,"Nama Pemain","#22d3ee");panel(ctx,start+(cardW+gap),y,cardW,"🏆",official?(localRank?`#${localRank}`:"#—"):"REMATCH",official?localLabel:"Practice","#fbbf24");panel(ctx,start+(cardW+gap)*2,y,cardW,"↗",official?nationalLabel:"NON-RANKED",official?(topPercent?`Top ${topPercent}% Indonesia`:"Peringkat Indonesia"):"Tidak ubah klasemen","#a78bfa");panel(ctx,start+(cardW+gap)*3,y,cardW,"◫",seasonLabel(props.submittedAt),"Season","#22d3ee")
...
export default function ResultShareCard(props:Props){
  ...
  async function share(){setBusy(true);try{const b=await blob();const f=new File([b],`ALZAVA-${props.nickname.replace(/[^a-z0-9]+/gi,"-")}-${props.battlePoint}.png`,{type:"image/png"});const url=props.participantPublicId?`${SITE_URL}/challenge?id=${encodeURIComponent(props.participantPublicId)}`:SITE_URL;const text=props.rankedAttempt===false?`Saya meraih ${Number(props.battlePoint).toLocaleString("id-ID")} Battle Point di Rematch ALZAVA. Bisa lewati skorku? ⚔️`:`Saya meraih ${Number(props.battlePoint).toLocaleString("id-ID")} Battle Point resmi di ALZAVA. Bisa lewati skorku? ⚔️`;if(navigator.canShare?.({files:[f]})&&navigator.share)await navigator.share({title:"ALZAVA Battle Point",text,url,files:[f]});else if(navigator.share)await navigator.share({title:"ALZAVA Battle Point",text,url});else{await navigator.clipboard.writeText(`${text} ${url}`);setCopied(true);setTimeout(()=>setCopied(false),1600)}}catch{}finally{setBusy(false)}}
