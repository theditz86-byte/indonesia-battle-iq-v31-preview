...
async function loadOfficialEntry(publicId:string){
  ...
  const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error("Skor resmi belum dapat dimuat.");return (d?.challenge_entry||null) as ChallengeEntry|null
}
...
async function buildShareCard(participant:Participant,entry:ChallengeEntry,link:string){
  ...
  const glow=ctx.createRadialGradient(540,650,40,540,650,640);glow.addColorStop(0,"rgba(34,211,238,.30)");glow.addColorStop(.55,"rgba(99,102,241,.16)");glow.addColorStop(1,"rgba(2,6,23,0)");ctx.fillStyle=glow;ctx.fillRect(0,0,1080,1400)
  ...
  try{const logo=await loadCanvasImage("/brand/alvaza-logo-new.svg");ctx.drawImage(logo,462,70,156,156)}catch{}
  ...
  if(avatar){try{const img=await loadCanvasImage(avatar);const scale=Math.max(264/img.naturalWidth,264/img.naturalHeight);const sw=264/scale,sh=264/scale;ctx.drawImage(img,(img.naturalWidth-sw)/2,(img.naturalHeight-sh)/2,sw,sh,408,703,264,264)}catch{ctx.fillStyle="#4338ca";ctx.fillRect(408,703,264,264)}}else{ctx.fillStyle="#4338ca";ctx.fillRect(408,703,264,264)}
  ctx.restore();ctx.strokeStyle="#fbbf24";ctx.lineWidth=9;ctx.beginPath();ctx.arc(540,835,136,0,Math.PI*2);ctx.stroke()
  if(!avatar){ctx.fillStyle="#fff";ctx.font="900 78px Arial";ctx.fillText(initials(participant.nickname||entry.nickname||"BP"),540,862)}
  ctx.fillStyle="#fff";ctx.font="900 52px Arial";ctx.fillText(participant.nickname||entry.nickname||"Pemain",540,1050)
  ctx.fillStyle="#94a3b8";ctx.font="700 24px Arial";ctx.fillText([entry.regency_name||participant.regency_name,entry.province_name||participant.province_name].filter(Boolean).join(" · ")||"Indonesia",540,1095)
  ...
  ctx.fillStyle="#94a3b8";ctx.font="700 19px Arial";ctx.fillText(link.replace(/^https?:\/\//,""),540,1785);ctx.fillStyle="#64748b";ctx.font="700 18px Arial";ctx.fillText("Raih Poin. Taklukkan Peringkat.",540,1840)
  return canvas
}
...
export default function ShareChallengePage(){
  ...
  async function downloadCard(){if(!participant||!entry||!publicId)return;const link=buildChallengeUrl(publicId,"card");const canvas=await buildShareCard(participant,entry,link);const a=document.createElement("a");a.href=canvas.toDataURL("image/png",.95);a.download=`alzava-battle-point-${(participant.nickname||"player").replace(/[^a-z0-9]+/gi,"-").toLowerCase()}.png`;a.click();setNotice("Kartu tantangan berhasil dibuat.")}
  async function nativeShare(){if(!participant||!entry||!publicId)return;const link=buildChallengeUrl(publicId,"native");const canvas=await buildShareCard(participant,entry,link);const blob=await canvasToBlob(canvas);const file=new File([blob],"alzava-battle-point.png",{type:"image/png"});const payload={title:"ALZAVA Battle Point",text:`Saya dapat ${score.toLocaleString("id-ID")} Battle Point. Coba kalahkan skorku!`,url:link,files:[file]};void trackReferralShare("native");if(navigator.share){try{if(!navigator.canShare||navigator.canShare({files:[file]}))await navigator.share(payload);else await navigator.share({title:payload.title,text:payload.text,url:payload.url})}catch{}}else{await navigator.clipboard.writeText(link);setNotice("Link tantangan sudah disalin.")}}
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
        <div className="overflow-hidden rounded-[2rem] border border-cyan-300/15 bg-[#07162f]/90 shadow-2xl">
          <div className="bg-gradient-to-br from-indigo-500/20 via-transparent to-cyan-400/10 p-7 text-center sm:p-9"><p className="text-xs font-black uppercase tracking-[.2em] text-cyan-300">Mesin Penyebaran Organik</p><h1 className="mt-3 text-3xl font-black sm:text-5xl">Bagikan. Tantang. Ajak Mereka Masuk Ranking.</h1><p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-400">Setiap link milik Anda dapat dilacak. Orang yang membuka, menerima tantangan, lalu mendaftar akan masuk statistik referral Anda.</p></div>
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-[2rem] border border-white/10 bg-slate-950/45 p-6 sm:p-7"><div className="flex items-center gap-2"><Users className="h-5 w-5 text-cyan-300"/><h2 className="text-xl font-black">Cara paling efektif</h2></div><div className="mt-4 space-y-3 text-sm leading-6 text-slate-400"><p><b className="text-white">1.</b> Bagikan kartu ke Status WhatsApp dengan kalimat: <span className="text-cyan-200">“Saya dapat {score||"…"} Battle Point. Bisa kalahkan?”</span></p><p><b className="text-white">2.</b> Jangan jelaskan terlalu panjang. Biarkan rasa penasaran membawa mereka ke link tantangan.</p><p><b className="text-white">3.</b> Sebarkan lagi setelah posisi ranking Anda berubah agar kontennya tidak terasa sama.</p></div></section>
