...
export default function AccountPage() {
  ...
  const action = useMemo(() => {
    ...
    if (paidCredits > 0) return { href: "/battle-test", label: `Mulai tes · kredit ${paidCredits}x` }
    return { href: "/payment", label: "Tambah 1 percobaan · Rp5.000" }
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_15%_0%,rgba(55,115,255,.20),transparent_34rem),radial-gradient(circle_at_90%_15%,rgba(42,210,255,.10),transparent_28rem),linear-gradient(180deg,#020817_0%,#07142f_46%,#050d20_100%)] text-white">
      ...
      <section className="mx-auto max-w-5xl px-5 py-10 sm:py-14">
        <div className="mb-7">
          ...
          <p className="mt-4 max-w-3xl leading-7 text-slate-300">Kelola akun, Nama Arena, foto profil, dan kuota percobaan. Setiap peserta mendapat <strong className="text-white">2x percobaan gratis per season</strong>. Setelah itu, setiap tambahan 1 percobaan adalah Rp5.000.</p>
        </div>
          ...
          <div className="grid gap-5">
            <section className="rounded-3xl border border-white/15 bg-[#0a1a37]/90 p-6 shadow-2xl sm:p-8">
              ...
              <div className="mt-7 grid gap-3 sm:grid-cols-4">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4"><span className="text-xs text-slate-400">Kredit berbayar</span><strong className="mt-1 block text-3xl font-black">{paidCredits}x</strong></div>
              </div>
              <p className="mt-4 text-sm text-slate-400">Total percobaan yang masih dapat dipakai sekarang: <strong className="text-white">{totalRemaining}x</strong>. Setelah 2x gratis habis, setiap Rp5.000 yang disetujui admin membuka tepat 1 percobaan tambahan.</p>
            </section>
